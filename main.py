...
def health():
    has_yt_cookies = bool(
        ...
        os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), "youtube_cookies.txt"))
    )
    has_ig_cookies = bool(
        os.environ.get("INSTAGRAM_COOKIES") or
        os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), "instagram_cookies.txt"))
    )
    return jsonify({
        "status": "ok",
...
def youtube_info():
    ...
    try:
        ...
        if not thumbnail and info.get("thumbnails"):
            thumbs = sorted(info["thumbnails"], key=lambda t: t.get("preference", 0) or 0)
        ...
        if "Sign in" in msg or "bot" in msg.lower() or "cookies" in msg.lower():
            return jsonify({"error": "YouTube bot check failed. Add YOUTUBE_COOKIES env var on Render."}), 403
...
def youtube_audio():
    ...
    try:
        ...
        if "Sign in" in msg or "bot" in msg.lower():
            return jsonify({"error": "YouTube bot check failed. Add YOUTUBE_COOKIES env var."}), 403
        return jsonify({"error": f"yt-dlp error: {msg[:300]}"}), 500
    except Exception as e:
        return jsonify({"error": f"Server error: {str(e)[:200]}"}), 500
...
def instagram_info():
    ...
    try:
        ...
        if "login" in msg.lower() or "private" in msg.lower():
            return jsonify({"error": "This Instagram account is private or requires login."}), 403
        if "not found" in msg.lower():
            return jsonify({"error": "Instagram post not found. Make sure the account is public."}), 404
        ...
        return jsonify({"error": f"Could not extract Instagram video: {msg[:200]}"}), 500
